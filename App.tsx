...
/**
 * Mendapatkan tanggal saat ini dalam format YYYY-MM-DD berdasarkan waktu lokal (WIB).
 */
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 bg-white p-6 rounded-3xl shadow-sm border border-slate-100 gap-4">
           <div>
             ...
             <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Ogah Foundation Control</p>
           </div>
           <div className="flex gap-3 w-full md:w-auto">
             <button 
               onClick={onRefresh} 
               disabled={isRefreshing}
               className="flex-1 md:flex-none px-6 py-3 bg-slate-100 text-slate-600 rounded-xl font-bold hover:bg-slate-200 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
             >
              ...
              {[
                ...
                { id: 'settings', label: 'Pengaturan', icon: <LucideSettings size={18}/> },
                 ...
                 <button 
                   ...
                   className={`w-full text-left px-6 py-4 rounded-2xl font-bold transition-all flex items-center gap-3 ${activeTab === tab.id ? 'bg-slate-800 text-white shadow-lg shadow-slate-200' : 'bg-white text-slate-500 hover:bg-slate-100'}`}
                 >
                   {tab.icon}
                   {tab.label}
                 </button>
           ...
           <div className="lg:col-span-3 bg-white p-6 md:p-8 rounded-[2.5rem] shadow-xl min-h-[60vh] border border-slate-100">
              {activeTab === 'overview' && (
                 <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <h3 className="text-2xl font-bold tracking-tight">Statistik Yayasan</h3>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                       ...
                       <div className="p-6 bg-rose-50 rounded-3xl border border-rose-100">
                          <p className="text-[10px] font-black text-rose-600 uppercase tracking-widest">Total Keluar</p>
                       ...
                       <div className="p-6 bg-blue-50 rounded-3xl border border-blue-100">
                          <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Saldo Tersisa</p>
              ...
              {activeTab === 'donations' && (
                 <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <h3 className="text-2xl font-bold tracking-tight">Verifikasi Masuk</h3>
                         ...
                         data.pendingIncomes.map(item => (
                           <div key={item.id} className="p-5 bg-slate-50 rounded-3xl border border-slate-100 flex items-center justify-between gap-4">
                              ...
                              <div className="flex gap-2">
                                <button onClick={() => approveDonation(item.id)} className="p-3 bg-emerald-500 text-white rounded-xl"><LucideCheck size={18}/></button>
                                <button onClick={() => rejectDonation(item.id)} className="p-3 bg-rose-500 text-white rounded-xl"><LucideX size={18}/></button>
                              </div>
                           </div>
                         ))
                       )}
...
const App: React.FC = () => {
  ...
  const fetchData = useCallback(async (isManual = false) => {
    ...
    try {
      ...
      if (cloudData) {
        // --- SINKRONISASI SEMUA TIPE PEMASUKAN ---
        const cleanedIncomes = (cloudData.incomes || []).map((i: any) => ({
           ...i, 
           amount: Number(i.amount) || 0,
           date: String(i.date)
        }));

        const cleanedPending = (cloudData.pendingIncomes || []).map((i: any) => ({
           ...i, 
           amount: Number(i.amount) || 0,
           date: String(i.date)
        }));

        const cleanedExpenses = (cloudData.expenses || []).map((e: any) => ({
           ...e, 
  ...
  const handleConfirmDonation = async (e: React.FormEvent) => {
    ...
    const localPending = { ...newPending, proofImage: confImagePreview || undefined };
    setTimeout(() => {
      setData(prev => ({ ...prev, pendingIncomes: [...prev.pendingIncomes, localPending] }));
    ...
    <div className={`min-h-screen ${layout.themeMode === 'soft' ? 'bg-slate-100' : 'bg-slate-50'} text-slate-900 transition-all duration-500 pb-12`} style={{ fontFamily: layout.fontFamily }}>
      ...
      {isDataLoading && (
        <div className="fixed inset-0 z-[100] bg-white/95 backdrop-blur-md flex flex-col items-center justify-center">
          <LucideLoader2 className={`w-12 h-12 text-${primaryColorClass}-600 animate-spin mb-4`} />
      ...
      <nav className="fixed top-0 left-0 w-full z-50 bg-white/80 backdrop-blur-xl border-b border-slate-100 py-4 px-6 shadow-sm">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
            ...
            <div className="w-12 h-12 rounded-2xl flex items-center justify-center overflow-hidden bg-white shadow-md border border-slate-50 p-1">
              <img 
                ...
                onError={(e) => {
                  ...
                  if (e.currentTarget.src.includes('Logo/ogah.png')) {
                    e.currentTarget.src = 'Logo/ogah.png';
                  }
                }}
              />
            </div>
            <span className="font-black text-xl tracking-tighter text-slate-800">
              Ogah<span className={`text-${primaryColorClass}-600`}>Foundation</span>
            </span>
          </div>
          ...
          <div className="hidden md:flex items-center gap-8 font-black uppercase text-[11px] tracking-widest text-slate-400">
            ...
            <button onClick={() => setCurrentView('articles')} className={`transition-all hover:scale-105 ${currentView === 'articles' ? `text-${primaryColorClass}-600` : 'hover:text-slate-600'}`}>Warta</button>
          ...
          <div className="space-y-24">
            <section className="px-6 max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-16 py-12 md:py-24">
              <div className="flex-1 space-y-8 text-center md:text-left">
                <div className={`inline-flex px-4 py-2 bg-${primaryColorClass}-50 text-${primaryColorClass}-600 rounded-full font-black text-xs uppercase tracking-widest animate-bounce-subtle`}>Kemanusiaan Tanpa Sekat</div>
                ...
                <div className="flex flex-wrap gap-4 justify-center md:justify-start pt-4">
                  <button onClick={() => setCurrentView('transparency')} className={`px-10 py-5 bg-${primaryColorClass}-600 text-white rounded-2xl font-black shadow-2xl shadow-${primaryColorClass}-100 hover:scale-105 transition-all flex items-center gap-3 text-lg`}>
          ...
          <div className="max-w-6xl mx-auto px-6 space-y-12 pb-24">
             <div className="text-center space-y-4">
              ...
              <div className="flex flex-col items-center gap-4">
                 <p className="text-slate-500 max-w-lg mx-auto">Sinkronisasi langsung dari pembukuan yayasan. Setiap rupiah yang Anda amanahkan tercatat nyata.</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in slide-in-from-bottom-8 duration-700">
                ...
                <div className={`bg-slate-900 p-8 rounded-3xl shadow-2xl flex flex-col justify-between relative overflow-hidden group min-h-[160px]`}>
                  <div className="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-full -mr-16 -mt-16 group-hover:scale-150 transition-transform duration-700"></div>
                  <div className="relative z-10">
                    <p className="text-slate-400 text-[11px] font-black uppercase tracking-widest mb-2">Saldo Tersisa Saat Ini</p>
                  ...
                  <div className="mt-4 flex items-center gap-2 text-amber-400 font-bold text-xs relative z-10 uppercase tracking-widest">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
                          ...
                          [...data.incomes].reverse().map((item) => (
                            <div key={item.id} className="flex justify-between items-center p-6 bg-slate-50 rounded-2xl border border-slate-100 hover:border-emerald-200 transition-all hover:translate-x-1">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="font-black text-slate-800 text-base">{item.donorName}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="font-black text-emerald-600 text-base">{formatCurrency(item.amount)}</p>
                                </div>
                            </div>
                          ))
                       )}
                  ...
                  <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-50 h-fit">
                    <h3 className="text-xl font-black text-slate-800 mb-6 flex items-center gap-3 uppercase tracking-tight">
                      <div className="w-10 h-10 rounded-2xl bg-violet-100 flex items-center justify-center text-violet-600 shadow-sm"><LucideArrowRight className="w-5 h-5 -rotate-45" /></div>
                      Riwayat Pengeluaran
                    </h3>
                    <div className="space-y-4 max-h-[600px] overflow-y-auto scrollbar-hide pr-2">
                       {data.expenses.length === 0 ? (
                          ...
                          [...data.expenses].reverse().map((item) => (
                            <div key={item.id} className="flex justify-between items-center p-6 bg-slate-50 rounded-2xl border border-slate-100 hover:border-violet-200 transition-all hover:translate-x-1">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-full bg-white border-2 border-violet-50 flex items-center justify-center text-violet-600 font-black text-sm shadow-sm">
                                      {item.itemName.charAt(0).toUpperCase()}
                                    </div>
                                    <div>
                                        <p className="font-black text-slate-800 text-base">{item.itemName}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="font-black text-violet-500 text-base">{formatCurrency(item.unitPrice * item.qty)}</p>
      ...
      {showQrisModal && (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-[420px] rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300 flex flex-col max-h-[95vh] p-8">
            ...
            <div className="overflow-y-auto pr-2 scrollbar-hide flex-grow mt-2 text-center">
              <div className={`inline-flex p-4 bg-${primaryColorClass}-50 text-${primaryColorClass}-600 rounded-2xl mb-4 shadow-sm`}><LucideQrCode className="w-8 h-8" /></div>
              <h3 className="text-2xl font-black text-slate-800 tracking-tight mb-4">Donasi Cepat</h3>
              <div className="bg-white p-4 rounded-3xl border-2 border-slate-50 shadow-inner inline-block mb-6"><img src={layout.qrisImageUrl} alt="QRIS" className="w-48 h-48 rounded-xl" /></div>
              ...
              <div className="border-t border-slate-100 pt-6 space-y-4 text-left">
                ...
                {submitSuccess ? (
                  <div className="bg-emerald-50 text-emerald-700 p-8 rounded-3xl text-center border border-emerald-100 animate-in zoom-in">
                    ...
                    <h5 className="font-black text-lg">Konfirmasi Terkirim!</h5>
                  ...
                  <form onSubmit={handleConfirmDonation} className="space-y-4">
                    <input type="text" placeholder="Nama Anda (Opsional)" value={confName} onChange={(e) => setConfName(e.target.value)} className="w-full px-5 py-4 rounded-2xl border-2 border-slate-50 focus:border-slate-800 outline-none font-bold bg-slate-50/50" />
                    <input required type="number" placeholder="Nominal Rp" value={confAmount} onChange={(e) => setConfAmount(e.target.value)} className="w-full px-5 py-4 rounded-2xl border-2 border-slate-50 focus:border-slate-800 outline-none font-black bg-slate-50/50 text-lg" />
                    ...
                    <button type="submit" disabled={isSubmitting} className={`w-full py-5 bg-${primaryColorClass}-600 text-white rounded-2xl font-black shadow-xl active:scale-95 transition-all text-lg flex items-center justify-center gap-3 disabled:opacity-50`}>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-sm rounded-3xl shadow-2xl p-10 animate-in zoom-in duration-300">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <input autoFocus type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full px-6 py-4 rounded-xl border-2 border-slate-50 focus:border-slate-800 outline-none font-bold bg-slate-50/50" placeholder="Username" />
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-6 py-4 rounded-xl border-2 border-slate-50 focus:border-slate-800 outline-none font-bold bg-slate-50/50" placeholder="Password" />
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button type="submit" className={`flex-1 py-4 bg-slate-900 text-white font-black rounded-xl shadow-lg text-sm uppercase tracking-widest`}>Masuk</button>
              </div>
            </form>
          </div>
        </div>
